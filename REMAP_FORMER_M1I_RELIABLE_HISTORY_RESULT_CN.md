...
只有“成功调用后被覆盖”再次占主导，才实现无硬 counter 的连续 hysteresis/retention；若主要是 attention 混合导致地址不够纯，则研究可微的 confidence-conditioned transport。两条路线都必须继续使用唯一 content HPC，不增加 context slot 或第二套 fast weights。
