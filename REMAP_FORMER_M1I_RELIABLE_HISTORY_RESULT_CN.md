...
后续 G3 已在全新 seed17181 完成：聚合 all-token 与 return-conflict 梯度 cosine 为 `-0.9579`，前者压低 100% return 段 transport、后者提高 100%；但逐 batch 负 cosine 仅 `9/16`，未过 `0.75` 稳定门，冻结状态为 `MIXED_GRADIENT_CREDIT`。因此仍不能直接换 loss，下一步是新 seed 的 probe 级信用审计。raw `pfc_hidden` 仍不作为 key；任何 native hidden caller 继续需要独立协议。
