...
后续 G3/G4 在两个新 seed 上确认 all-token 与 return 聚合梯度强反向，但 G4 的 return 内部仍有 112 support / 16 reject，且六特征预测 support/source error 的 BA 仅 `0.5000/0.5961`，因此停止 event-balanced 训练。G5 又在 seed19184 复现 88 support / 40 reject；correct-context oracle 为 `0.8594`，但 reject 被分成 source-correct 8、downstream 8、archival 2、shared-scalar 6、local-geometry 16，没有类别达到 50% 主导门。冻结状态 `MIXED_CONTEXT_VALUE_ADDRESS_GEOMETRY`，不再训练 M1j。raw `pfc_hidden` 仍不作为 key；任何 native hidden caller 继续需要独立协议。
...
- 回归：整套 `test_remap_former*.py` 回归 `171 passed`。当前 Anaconda 必须先加载 NumPy 再加载 Torch，避免两份 OpenMP runtime 冲突；未使用不安全的 `KMP_DUPLICATE_LIB_OK`。全仓测试另被旧 `memlab`/`hippoformer` 可选依赖缺失阻断在收集阶段。
