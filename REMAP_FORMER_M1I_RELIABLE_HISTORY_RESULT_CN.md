...
> 结论先行：M1i 把独立 K=8 return-conflict 从冻结 M1f 的 `0.3906` 提高到 `0.6406`，并通过增益、必要性、clean 保持和 context identity 四道门；但没有达到预注册绝对门槛 `0.75`，因此正式分类为 `M1I_RELIABLE_HISTORY_PILOT_REJECTED`。不扩三训练 seed，不在盲测 seed 9931 上调参，正式模型继续保持 frozen M1b。

## 1. 这一版到底是什么

M1i 不是新建一套内容记忆，也没有显式 context slot。它在 M1f 上只增加一个小型 PFC 历史调用器：
...
- **正式状态**：frozen M1b 仍是当前最干净、已完成预算匹配比较的正式版本；M1f/M1h/M1i 是定位 context re-entry 的机制链。

## 6. 下一步

下一步不应立刻加参数或加新记忆。先在**全新、预注册的 dev seed**上做只读错误分解，逐个 return probe 标记：

1. 正确 signature 历史是否存在；
2. soft attention 是否把最大质量给了正确 proposal；
3. 正确 call 后是否被后续 unmatched proposal 覆盖；
4. return 时 context 已正确但 covariance HPC 仍答错的比例；
5. no-call、normal 与 force-call 的错误是否落到同一类。

只有“成功调用后被覆盖”再次占主导，才实现无硬 counter 的连续 hysteresis/retention；若主要是 attention 混合导致地址不够纯，则研究可微的 confidence-conditioned transport。若错误分解显示 action-only structural caller 本身缺信息，下一份独立协议才允许让 query/controller 读取严格因果的 `pfc_hidden`，同时保留 fixed structural key 与不可递归写回的 archival value。所有路线都必须继续使用唯一 content HPC，不增加 context slot 或第二套 fast weights。
...
- 回归：本课题 `test_remap_former*.py` 共 `126 passed`；全仓测试另被旧 `memlab`/`hippoformer` 可选依赖缺失阻断在收集阶段。
