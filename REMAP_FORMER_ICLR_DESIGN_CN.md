...
> ICLR 风格模型设计与预注册实验计划，评审合并冻结稿 v8，2026-07-14
...
- M1j 已执行 G2 指定的 confidence-conditioned transport 分支：在 frozen M1i call 与 context 混合之间加入 77 参数、±3 bounded logit residual，其他全部冻结，唯一目标仍是均匀 all-token sensory CE。seed31415 固定 600 步把 transport strength 从 `0.450` 压到 `0.099`，100% history events 为负 shift；CE `1.5002→1.4118`，K8 monitor `0.625→0.125`。全新 seed16180 盲测中 M1f/source-M1i/M1j/disabled 为 `0.3750/0.7344/0.0781/0.7344`，disabled prediction/context diff 均为 `0`。七门只过 clean 和两条等价门，冻结分类 `M1J_CONFIDENCE_TRANSPORT_PILOT_REJECTED`。下一步只允许全新 seed 的只读 gradient-credit 审计，不得在 seed16180 改 loss、约束或结构。见 `reports/REMAP_FORMER_M1J_CONFIDENCE_TRANSPORT_RESULT_CN.md`。
...
| M1j pilot | Frozen M1i + 77 参数 bounded transport residual | 检验均匀 sensory CE 能否学会 context 精确运输 | source/disabled 0.7344，M1j 0.0781；100% negative shift；七门只过三门，停止且先审计 gradient credit |
...
截至 2026-07-14，原 M1 的 raw 与 EMA arm 均只有 `2/3` seeds 通过，因此原 M2/M3 路线仍停止。随后冻结诊断定位到 HPC 内容串扰，单机制 M1b 已通过 substrate、8-seed headline 与预算匹配 Hippoformer 终审；这不追溯修改原 M1 停止结论。新奇容量上，M1c 与 M1d 的 hard event classifier 均在 dev gate 被拒绝，但后续 160-probe 同 episode 诊断表明 M1d soft/force-proposal 分别达到 `0.6125/0.6625`，明显高于 M1b 的 `0.4750`；低到 `0.3000` 的只是 hard threshold。新增 slot-free associative context memory 的 M1e delta/covariance arm没有正向 recall 因果增益，因此停止。由此提炼出的零新增参数 M1f 在冻结三 seed dev gate 上从 M1b 聚合 `0.4125` 提升到 `0.6083`，逐 seed 增益均为正，但独立 K=8 confirmation 仅为 `0.4010`，不进入 formal。冻结 oracle 将正确历史 context 和 exact address 都恢复到约 `0.9844`，把瓶颈锁定为 context identity retrieval。M1g 的 content-HPC transpose/least-squares 逆恢复均未提供可辨识信号。M1h/M1i 建立了 auxiliary action-only history caller 的有效性与边界；G2 把主要残差定位到 context 精度。M1j 随后证明，仅靠均匀 all-token sensory CE 训练一个 signed confidence transport 会学成全局负 shift：来源/disabled 为 `0.7344`，M1j 只有 `0.0781`，且实现等价差为零。该结果拒绝当前 transport+loss 组合，但尚未区分“return gradient 与 all-token gradient 冲突”和“transport 几何本身错误”。因此下一步只做冻结的 gradient-credit 审计；正式模型仍为未额外微调的 M1b，formal split 继续封存。
...
30. M1j 已按独立协议完成：77 参数 confidence transport、seed31415、600×batch4、固定 final checkpoint。训练把 transport strength `0.450→0.099`，所有 history event 的 shift 均为负。全新 seed16180 的 M1f/source-M1i/M1j/disabled 为 `0.3750/0.7344/0.0781/0.7344`，disabled prediction/context 最大差均为 `0`。冻结分类 `M1J_CONFIDENCE_TRANSPORT_PILOT_REJECTED`；不铺三 seed，不在 seed16180 调参。下一步另立新 seed 的 gradient-credit 只读审计，再决定是否允许 event-balanced sensory objective。
