...
> ICLR 风格模型设计与预注册实验计划，评审合并冻结稿 v4，2026-07-14
...
截至 2026-07-14，原 M1 的 raw 与 EMA arm 均只有 `2/3` seeds 通过，因此原 M2/M3 路线仍停止。随后冻结诊断定位到 HPC 内容串扰，单机制 M1b 已通过 substrate、8-seed headline 与预算匹配 Hippoformer 终审；这不追溯修改原 M1 停止结论。新奇容量上，M1c 与 M1d 的 hard event classifier 均在 dev gate 被拒绝，但后续 160-probe 同 episode 诊断表明 M1d soft/force-proposal 分别达到 `0.6125/0.6625`，明显高于 M1b 的 `0.4750`；低到 `0.3000` 的只是 hard threshold。新增 slot-free associative context memory 的 M1e delta/covariance arm 没有正向 recall 因果增益，因此停止。由此提炼出的零新增参数 M1f 在冻结三 seed dev gate 上从 M1b 聚合 `0.4125` 提升到 `0.6083`，逐 seed 增益为 `+0.2625/+0.2000/+0.1250`，五个 gates 全过。初轮 K=8 的 `-0.0417` 在全新 384-probe confirmation 中未复现：M1b/M1f 为 `0.3177/0.4010`，3/3 seeds 正向；balanced schedule 比等次数随机高 `0.1224`，但只比 delay+6 高 `0.0313`。机制表述冻结为“结构约束且时间容忍较宽的 hold/refresh”，不声称精确边界检测。由于绝对 `0.4010` 仍不足，M1f 不进入 formal；同一 384 probes 的冻结 oracle 分解中，正确历史 context 与精确地址都把 fused recall 恢复到 `0.9844`，错误历史 context 为 `0`，精确地址 raw HPC 为 `0.9896`。随后 M1g 分别用 transpose backprojection 与 rolling least squares 检验现有 content HPC 的反向 context 信号；两者在新 K=8 dev episodes 上均未通过 identity、shuffled 和 downstream recall gates，且 exact replay error 都为 `0`，因此该路线正式停止。下一候选只允许解决 Transformer/PFC 对自身历史 context 的因果调用，不能重做已健康的内容 HPC，也不能增加 context slots 或第二套 fast-weight memory。formal split 未访问；当前正式模型仍为未额外微调的 frozen converted M1b。
