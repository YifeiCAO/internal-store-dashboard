...
> ICLR 风格模型设计与预注册实验计划，评审合并冻结稿 v19，2026-07-16
...
- M1p 按该 objective-level 分支实现单 HPC likelihood-marginal router：唯一 covariance HPC 始终按 frozen source 每 token 写一次，source/archive 只从同一个写前 memory state 各读一次；285 个 router 参数用两条冻结 sensory likelihood 的概率边缘化训练，仍无 room/return/event/route 标签、slot 或第二套 fast weights。v1 G0 因无条件 return sign-rate `0.6063<0.65` 被 11/12 gate 拒绝且未训练；v2 用全新 seed 与 utility-conditioned gate 检查真正 route-changing probes，memory/source-benefit 占比为 `0.2063/0.5259`、likelihood alignment 均 `1.0`，16/16 gates 通过。seed1437151 固定 400 步后，original-K8 hard/source 为 `0.5938/0.5000`，soft marginal diagnostic 为 `0.6563`；hard call/correct-room/distractor-source 为 `0.7813/0.6875/0.8805`，clean `0.9422` vs source `0.9500`。9 个 unlock gates 过 7 个，absolute 与 `+0.10` gain 门失败，冻结状态 `M1P_TRAINING_GATE_REJECTED`，fresh blind seed1457151 未打开。结论是 marginal likelihood 已修复低召回，但剩余瓶颈转为 soft-train/hard-deploy mismatch；soft 结果不可追认为本协议成功。见 `reports/REMAP_FORMER_M1P_LIKELIHOOD_ROUTER_RESULT_CN.md`。
- M1q 保持 M1p 的单 HPC、14 个 causal features、285 参数与固定 400 步，只把训练前向改成 straight-through Binary-Concrete sampled hard route。Fresh G0 seed1527151 的 source/archive/oracle 为 `0.7125/0.9500/0.9688`，16/16 gates 通过；CUDA smoke 24/24、当时 ReMAP 回归 242 passed。但 step400 original-K8 的 source/hard/soft 均为 `0.28125`，deterministic call/correct-room 为 `0/0`，9 个 unlock gates 只过 5 个，blind seed1557151 未打开。只读 regret audit 显示 18,966 个 history tokens 的 source/archive regret mass 为 `48,118.4/9,167.1`，source 方向 `5.249x`；最终 history 最大 logit 仍为 `-1.4223`。冻结分类 `M1Q_TRAINING_GATE_REJECTED`：hard-consistency 本身不能抵消训练分布中的 source-regret 支配。
- M1r 在完全相同的架构、router 参数、初始 bias、训练数据与预算上，只把唯一训练目标改为 class-balanced detached counterfactual preference BCE：当前 sensory target 仅在 loss 内比较两条冻结分支 NLL，不进入 forward；archive/source-preferred 两组分别取均值后 1:1 合并，不使用 room/return/event/generator metadata。Fresh G0 seed1627151 为 18/18 PASS，preference fraction `0.2746/0.7249`；CUDA smoke 24/24、最终 ReMAP 回归 250 passed。seed1637151 固定 step400 monitor 上 source/hard 为 `0.3438/0.7500`，gain `+0.4063`，call/correct-room/distractor-source `0.9688/0.9063/0.5621`，9/9 training gates 全过。一次性 blind seed1657151 上 source/disabled/hard 为 `0.4063/0.4063/0.7344`，gain `+0.3281`，clean drop `0.0031`，call/correct-room `0.8750/0.7656`；7 门过 6 门，唯一失败为 absolute `94/128=0.7344<0.75=96/128`。冻结状态 `M1R_COUNTERFACTUAL_ROUTER_REJECTED`，不得因少 2 probes 重跑、扩 seed 或追认；正式 M1b 保持不变。见 `reports/REMAP_FORMER_M1Q_M1R_HARD_ROUTING_RESULT_CN.md`。
- M1s 随后按 fresh seed1727151、只读、128 episodes/256 return-conflict probes 的冻结协议分解 M1r 剩余错误；未重跑或逐 probe 使用 blind1657151。Task health 6/6、implementation 10/10、ReMAP 回归 258 passed。Fresh source/hard/fixed-archive/oracle 为 `0.3672/0.6875/0.7266/0.7969`，hard 增益 `+0.3203`。80 个 hard errors 精确分成 missed/harmful call `16/14`、两分支都错且 archive room 错 `28`、两分支都错但 archive room 对 `22`；合并 share 为 router/event-identity/correct-room-downstream `0.375/0.350/0.275`，均低于预注册 `0.50` dominance 门。Exactly-one-correct route accuracy `0.7727`，但 source-only retention 仅 `0.3000`；return preference BA/AUROC `0.5230/0.5950`。冻结分类 `M1S_MIXED_NO_SINGLE_MECHANISM`，分支 `STOP_MODEL_STACKING_AND_RETAIN_FORMAL_M1B`：不创建 M1t，不把三个局部补丁叠成不可归因的新模型，课题转入正式 M1b headline 与 M1p-q-r objective ablation 的论文收口。见 `reports/REMAP_FORMER_M1S_FRESH_ERROR_DECOMPOSITION_CN.md`。
...
40. M1p v2 已按独立 likelihood-routing 协议完成：单一 source-written covariance HPC、两条同状态写前 read、285 个 causal router 参数、seed1437151、400×batch4、固定 final checkpoint。v1 G0 以 11/12 gate 拒绝并保持未训练；v2 在全新 seed1427151 上以 utility-conditioned likelihood alignment 达到 16/16 PASS，CUDA implementation smoke 19/19、ReMAP 回归 233 passed。未打开 blind 的 original-K8 validation monitor 上 source/hard/soft-marginal/oracle 为 `0.5000/0.5938/0.6563/0.7188`，hard call/correct-room/distractor-source 为 `0.7813/0.6875/0.8805`，clean drop `0.0078`。9 个 unlock gates 通过 7 个，absolute `0.5938<0.65` 与 gain `0.09375<0.10` 失败；冻结状态 `M1P_TRAINING_GATE_REJECTED`，分支 `STOP_WITHOUT_OPENING_BLIND_OR_TUNING_MONITOR`，fresh blind seed1457151 保持未读。下一步必须在“hard-consistent 无标签调用”与“正式 soft latent marginal deployment”之间另立语义明确的新协议和全新 seeds，不得调本 monitor 的 hard threshold 或追认 soft diagnostic。
41. M1q 已按 objective-only 协议完成：straight-through Binary-Concrete、seed1537151、400×batch4、固定 final checkpoint，架构与 M1p 完全相同。Fresh G0 16/16、smoke 24/24、ReMAP 242 passed；但 final original-K8 source/hard/soft `0.28125/0.28125/0.28125`，hard call 为 `0`，training gates 5/9。Regret audit 将失败归因为 source/archive 累计 regret mass `48,118.4/9,167.1` 的 `5.249x` 失衡，而非实现或梯度符号错误。冻结状态 `M1Q_TRAINING_GATE_REJECTED`，blind1557151 未打开。
42. M1r 已按单一 counterfactual preference 协议完成：seed1637151、400×batch4、285 参数，target-derived branch preference 只存在于 detached training loss，不进入模型输入；两类分别平均后 1:1 合并。Fresh G0 18/18、smoke 24/24、ReMAP 250 passed。Final monitor source/hard `0.34375/0.75000`，9/9 unlock gates 全过，故依法只打开一次 blind1657151。Blind source/disabled/hard `0.40625/0.40625/0.734375`，gain `+0.328125`，clean drop `0.003125`，call/correct-room `0.875/0.765625`；7 门过 6 门，absolute 少 2/128 probes。冻结状态 `M1R_COUNTERFACTUAL_ROUTER_REJECTED`，分支 `STOP_M1R_WITHOUT_TUNING_AND_RETAIN_FORMAL_M1B`；不得重跑、扩 seed 或改阈值。
43. M1s 已在 fresh validation seed1727151 完成 256-probe 只读分解。Source/hard/archive/oracle `0.3672/0.6875/0.7266/0.7969`；80 errors 为 missed call 16、harmful call 14、wrong archive room ceiling 28、correct archive room ceiling 22，三机制 share `0.375/0.350/0.275`。Task 6/6、implementation 10/10，完整 state hash 不变，逐 probe partition 无遗漏；router 在 132 个 exactly-one-correct probes 上 route accuracy `0.7727`，archive-only call recall `0.8571`，source-only retention `0.3000`。冻结状态 `M1S_MIXED_NO_SINGLE_MECHANISM`，不得新建单机制 M1t；模型线收口并保留 formal M1b。
