...
| M1d exploratory | Frozen M1b + 多信号递归 context acceptance | 新奇干扰下的 context 重入 | proposal recall=1；precision>=0.99；recall>=0.95；误调用<=0.05/episode |
...
截至 2026-07-13，原 M1 的 raw 与 EMA arm 均只有 `2/3` seeds 通过，因此原 M2/M3 路线仍停止。随后冻结诊断定位到 HPC 内容串扰，单机制 M1b 已通过 substrate、8-seed headline 与预算匹配 Hippoformer 终审；这不追溯修改原 M1 停止结论。新奇容量上的 M1c/M1d 均在 dev gate 被拒绝，未访问 formal split。当前正式模型仍为未额外微调的 frozen converted M1b。
...
18. M1d 多信号递归 context 分支已完成五个单种子 train/dev arm；最终 EC-proposal/PFC-acceptance 的 proposal recall 为 `1.0`，但 precision/recall/误调用门失败。该分支停止于 dev，stress seed、formal validation/test 与多 seed 均未运行。
