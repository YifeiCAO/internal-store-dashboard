...
| M1d exploratory | Frozen M1b + EC proposal 时刻的 PFC context refresh | 新奇干扰下的 context 重入 | 固定同 episode paired return 增益>=10 pp；clean 不降超过2 pp；event precision 只作诊断 |
...
截至 2026-07-13，原 M1 的 raw 与 EMA arm 均只有 `2/3` seeds 通过，因此原 M2/M3 路线仍停止。随后冻结诊断定位到 HPC 内容串扰，单机制 M1b 已通过 substrate、8-seed headline 与预算匹配 Hippoformer 终审；这不追溯修改原 M1 停止结论。新奇容量上，M1c 与 M1d 的 hard event classifier 均在 dev gate 被拒绝，但后续 160-probe 同 episode 诊断表明 M1d soft/force-proposal 分别达到 `0.6125/0.6625`，明显高于 M1b 的 `0.4750`；低到 `0.3000` 的只是 hard threshold。新增 slot-free associative context memory 的 M1e delta/covariance arm 没有正向 recall 因果增益，因此停止。所有这些探索均未访问 formal split；当前正式模型仍为未额外微调的 frozen converted M1b。
...
20. M1e slot-free associative context fast-weight pilot 已完成并拒绝。160-probe 配对中 delta/covariance 为 `0.6625/0.6250`，其 no-recall 消融均为 `0.6625`；关联 recall 的净贡献为 `0/-0.0375`。下一版只允许把无额外记忆的 EC-proposal context refresh 写成独立干净候选，并先过三个预注册 dev seeds。
