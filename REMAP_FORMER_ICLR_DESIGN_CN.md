...
> ICLR 风格模型设计与预注册实验计划，评审合并冻结稿 v6，2026-07-14
...
- M1h 先用严格因果 hard caller 检验普通 PFC event-token history：G1 return-conflict 从 M1f `0.3438` 升至 `0.6406`，但 22/128 probes 在成功调用后被一次 unmatched proposal 覆盖；按现有 32-step window 保持的独立 G1b 新 seed gate 达到 `0.8438`，相对 M1f `+0.3750`、相对 shuffled `+0.7422`，六门全过。该结果只证明历史调用上界，不把 hard rule 升级为模型。
- M1h neural attention pilot 已完成并拒绝。只训练 3,467 个 PFC history Q/K 与三路 `hold/refresh/call` controller 参数，唯一目标为 sensory CE；固定 600 步训练内 dev return-conflict 达 `0.9688`。但全新 seed8928 K=8 盲测中，M1f/neural/no-call 为 `0.4062/0.1719/0.3906`，neural context pair 仅 `0.6875`，五门只过 clean。只读审计显示原始 signature top-1 命中 `1.0`，训练后 Q/K 仅 `0.5926`，且所有 probe 都在 attention max `<0.5` 时强制偏向 call；结论为 key 漂移、无 abstention 与递归 value 污染。M1h 不铺三 seed，正式模型仍为 M1b。见 `reports/REMAP_FORMER_M1H_HISTORY_ATTENTION_RESULT_CN.md`。
- M1i 针对上述三处失败只引入固定 normalized signature key、不可变 `base_context` archival value 和 bounded learned null；PFC/EC/place/content-HPC/fusion/decoder 全冻结，只训练 2,479 个 hold/refresh/null 参数，仍只有全 token sensory CE。固定 600 步后在全新 seed9931 的 128-probe K=8 盲测中，M1f/M1i/no-call/force-call 为 `0.3906/0.6406/0.2812/0.3125`；M1i 相对 M1f `+25.00 pp`、相对 no-call `+35.94 pp`，clean 无下降，context pair `0.8750`。但绝对 return 未达预注册 `0.75`，五门只过四门，冻结分类 `M1I_RELIABLE_HISTORY_PILOT_REJECTED`。不扩三 seed、不在 seed9931 上调参，正式模型仍为 M1b；下一步只允许在全新 dev seed 做调用→覆盖→recall 的只读错误分解。见 `reports/REMAP_FORMER_M1I_RELIABLE_HISTORY_RESULT_CN.md`。
...
| M1h diagnostic/pilot | Frozen M1f + causal PFC token-history attention | 不加 slot/第二套 fast weight 的 context 调用 | hard persistence 上界通过；3,467 参数 neural pilot 的新 seed K=8 五门只过 clean，停止且不铺多 seed |
...
24. M1g 双向 context G0/G0b 已在两个新 dev seed 上按顺序完成。Transpose causal pair accuracy 为 `0.6094`，仅比 shuffled 高 `0.0312`；rolling LS causal 为 `0.4844`，比 shuffled 低 `0.0781`，诊断 return-conflict 为 `0.5469`。两者 replay error 均为 `0`，证明失败来自信息不可辨识而非代码偏差。按冻结规则不实现 deployable M1g、不铺多 seed；下一步另立 Transformer/PFC 历史 attention 调用协议。
25. M1h G1/G1b 已按两个独立新 seed 完成。G1 hard causal caller 为 `0.6406`，128 probes 全部曾成功调用但 22 个被后续 proposal 覆盖；G1b 32-step persistence 为 `0.8438`，比 normal 高 `0.3750`、比 shuffled 高 `0.7422`，六门全过。它只解锁 neural pilot，不是最终模型。
26. M1h neural pilot 固定训练 seed1712、600×batch4、3,467 trainable parameters、单一 sensory CE、无 checkpoint 选择；训练内 dev 为 `0.9688`。全新 seed8928 K=8 盲测的 M1f/neural/no-call 为 `0.4062/0.1719/0.3906`，context pair 为 `0.6875`，冻结分类 `NEURAL_HISTORY_CALL_PILOT_REJECTED`；按协议不铺三 seed且不在 seed8928 上调参。
...
28. M1i 已按冻结协议完成：固定 key、archival value 隔离、连续 null、2,479 trainable parameters、seed1813、600×batch4、无 checkpoint 选择。全新 seed9931 K=8 盲测为 M1f/M1i/no-call/force-call `0.3906/0.6406/0.2812/0.3125`，五门过四门，仅绝对 `0.75` 门失败。按 post-result rule 停止三 seed 扩展，不使用 seed9931 调参；下一步另立新 seed 的只读 failure audit 协议。
