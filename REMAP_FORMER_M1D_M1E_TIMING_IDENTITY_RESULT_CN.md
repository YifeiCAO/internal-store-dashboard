...
4. **下一版应从简单机制出发**：由因果动作历史的 balanced sliding-window detector 产生结构刷新信号，PFC 在触发时采用当前 context；该 detector 不是 neural EC 输出，也不保证稀疏。不再加入第二套 context fast-weight memory，也不再用严格 boundary precision 作为主目标。
